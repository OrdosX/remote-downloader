...
app.post('/tasks', (req, res) => {
    ...
    req.on("end", () => {
        ...
        downloader.on("error", (err) => {
            tasks.set(id, {code: -1, progress: 0, errmsg: err.message, downloader: null});
        ...
        downloader.on("end", () => {
            ...
            if(t.code == 2) {
                ...
                tasks.set(id, {code: 0, progress: 100, errmsg: "", downloader: null});
                logger.succ(body.name, body.URL);
            }
        })
        res.json({code:0, ID:id});
...
app.get('/tasks/:id', (req, res) => {
    if(tasks.has(req.params.id)) {
        ...
        res.json({code: -2, progress: 0, errmsg: "任务不存在"});
    }
})
...
app.delete('/tasks/:id', (req, res) => {
    if(tasks.has(req.params.id)) {
        ...
        res.json({code: 0, errmsg: ""});
    } else {
        res.json({code: -2, errmsg: "任务不存在"});
    }
})
